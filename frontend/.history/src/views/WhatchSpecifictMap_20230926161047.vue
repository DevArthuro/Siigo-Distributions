...
onMounted(() => {
  ...
  // Agrega nodos
  graph.addNode(data.locations[0], { x: 0, y: 10, size: 5, label: "John", color: "blue" });
  graph.addNode(data.locations[1], { x: 10, y: 0, size: 3, label: "Mary", color: "red" });
  graph.addNode(data.locations[2], { x: 20, y: 5, size: 2, label: "Mary", color: "red" });
